<template>
  <g :transform="'translate(' +infoRect+')'">
    <rect v-if="isShowDataInfo && !fold" rx="20" ry="20" y="-5" width="410" height="210" fill="rgb(255,247,236)"
          stroke="rgb(103,169,207)"
          stroke-width="2"></rect>
    <rect v-if="isShowDataInfo && !fold" @click="foldClick" rx="2" ry="2" width="20" height="20" fill="rgb(255,247,236)"
...
export default {
  name: "dataflowInfo",
  props: ["flow", "vertexFlow", "xScale", "yScale", "boundHeight"],
  mounted() {
  },
  data() {
    return {
  ...
  methods: {
    ...
    initDataHeatMap() {
      ...
      let maxVal = 0, totalLen = d3.entries(calDataDis).length, totalLenSrc = d3.entries(srcDataDis).length
      let rects = [], calMachines = [], texts = []
      let xOff = 34, yOff = 34, lineRectWid = (364 - totalLen) / totalLen,
          colRectHeight = (164 - totalLenSrc) / totalLenSrc
  ...
  computed: {
    infoRect() {
      ...
      return [this.xScale(this.flow.times[(this.flow.times.length - 1) / 2]), diff > 0 ? this.yScale(0) - diff : this.yScale(0)]
    },
    height() {
      let maxBeforeHeight = this.flow.beforeProFlow[0]
