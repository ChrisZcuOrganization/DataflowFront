<template>
  <div style="width: 100%; height: 100%">
    <div v-if="isQueryFlow" style="width: 100%; height: 100%">
      <div style="width: 100%; height: 100%">
        <svg v-if="tasksLayoutDone && dataflow !== undefined" class="vertexFlowClass" style="width: 100%; height: 100%">
    ...
    <div v-if="!isQueryFlow" style="width: 100%; height: 100%">
      <div style="width: 100%; height: 100%; ">
        ...
        <!--          </path>-->
        <!--        </svg>-->
        <svg  class="vertexFlowClass" style="width: 100%; height: 100%;">
          <g id="vertexFlowTimeAxis" transform="translate(7,10)"></g>
          <svg v-if="tasksLayoutDone">
            <g transform="translate(5,20)">
              ...
              <g>
                ...
                <g v-for="(vertexFlow, idx) in vertexesFlow" :key="'i' + idx">
                  <dataflowInfo v-if="true" :flow="vertexFlow.flow"
  ...
  watch: {
    tasksLayoutDone(newV, oldV) {
      if (newV) {
        ...
        if (!this.isQueryFlow) {
          let boundWidth = this.$el.getBoundingClientRect().width - 10
          let yScale = d3.scaleLinear()
              .domain([0, this.dataflow.endTime - this.dataflow.startTime])
              .range([0, boundWidth])
          console.log(this.dataflow.startTime, this.dataflow.endTime, boundWidth)
          // console.log(0, this.queryFlow.times, this.startTime, this.endTime)
          d3.select(this.$el).select('#vertexFlowTimeAxis').call(d3.axisBottom(yScale).ticks(10).tickFormat(d => {
            return d / 1000 + "s"
          }).tickSize(3))
          d3.select(this.$el).select('#vertexFlowTimeAxis').selectAll('text').attr("font-size", "15px").attr("stroke-width", 1)
        }
      }
    }
  },
